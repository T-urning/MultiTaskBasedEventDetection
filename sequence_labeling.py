...
from data_prepare import (
    DuEventExtraction, convert_example_to_feature_multitask_event_type)
from losses import FocalLoss
from models.models import DMCNN, BiLSTM, BiLSTMCRF
...
parser.add_argument("--train_batch_size", type=int, default=64, help="Total examples' number in batch for training.")
...
def badcase_analysis(model, save_path, data_loader, metric):
    ...
    for idx, (input_ids, token_type_ids, seq_lens, labels, border_labels) in enumerate(data_loader):
        ...
        masks = [[1 if i < length else 0 for i in range(input_ids.size(1))] for length in seq_lens]
        masks = torch.FloatTensor(masks).to(args.device)
        result = model(input_ids, attention_mask=masks, labels=labels, is_predict=True)
        ...
        if check_result:
            ...
            unpad_predictions, unpad_labels, n_bound_false, n_class_false = check_result
            
            writer.write(f"{idx+1}:\t边界错 {n_bound_false} 个，类别错 {n_class_false} 个\n\n")
            writer.write(f"Gold:\t{unpad_labels[0]}\n")
            writer.write(f"Pred:\t{unpad_predictions[0]}\n\n")
            writer.flush()

            bound_false_total += n_bound_false
            class_false_total += n_class_false
    ...
    writer.close()
    logger.warning(f"{'*'*50}")
    false_total = bound_false_total + class_false_total
    description = f"触发词共错 {false_total} 个，其中边界错 {bound_false_total} 个，占比 {bound_false_total/false_total:.5f}"
...
def main(model_map):
    ...
    # LSTM 等模型直接使用 BERT 中的预训练词向量
    pretrained_word_embeddings = os.path.join(pretrained_model_path, 'bert_word_embeddings.pt')
    model = model_map[args.model_name].from_pretrained(
                model_path, **{'num_labels':num_labels, 'max_seq_len': args.max_seq_len, 
                'embedding_file': pretrained_word_embeddings, 'from_bert':True})
