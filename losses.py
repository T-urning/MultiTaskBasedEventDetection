...
class DiceLossWithIngnoreIndex(nn.Module):
    """
    ...
    def __init__(self,
                 ...
                 ignore_index: Optional[int] = -1000,
                 ohem_ratio: float = 0.0,
                 alpha: float = 0.0,
                 reduction: Optional[str] = "mean") -> None:
        super(DiceLossWithIngnoreIndex, self).__init__()
    ...
    def forward(self, input: Tensor, target: Tensor) -> Tensor:
        ...
        if logits_size != 1:
            loss = self._multiple_class(input, target, logits_size)
        else:
    ...
    def _multiple_class(self, input, target, logits_size):
        flat_input = input
        ...
        if self.ignore_index is not None:
            ...
            mask = (target != self.ignore_index).float() # (batch_size * seq_len, )
            target_without_ignore = (target * mask).long()
            flat_target = F.one_hot(target_without_ignore, num_classes=logits_size).float()
            mask = mask.view(-1, 1).expand_as(flat_input)
            flat_input = flat_input * mask
            flat_target = flat_target * mask
        else:
            mask = torch.ones_like(flat_input)
